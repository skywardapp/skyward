 ...
 * destination, and drives the whole §13.1 onboarding flow end to end -- welcome
 * -> location -> notification permission -> exact-alarm explainer -> rules
 * preview -> Upcoming -- rather than only the welcome step a user sees first.
 */
...
class MainActivityUiTest {
    ...
    // connectedAndroidTest runs every test method in this process without clearing app data
    // between them (no test orchestrator configured), so a method that finishes onboarding
    // would otherwise leave onboarding_done=true for whichever method the runner picks next.
    // Force the flag back to unset and recreate before every test so each one starts from a
    // genuine "fresh install" regardless of run order or what a previous method left behind.
    @Before
    ...
    /**
     * Drives every §13.1 onboarding step through its app-side "proceed" action --
     * entering a manual location rather than granting the coarse-location
     * permission, and skipping the notification/exact-alarm permission prompts
     * rather than driving the real OS dialogs an instrumented test can't click
     * deterministically (§10.2 promises the app stays fully usable either way) --
     * and asserts it lands on Upcoming with the bottom bar, then that a relaunch
     * (simulated via recreate, as [uiSurvivesActivityRecreation] does) skips
     * straight back to Upcoming instead of onboarding again.
     */
    @Test
    fun userCompletesOnboardingWithALocationAndReachesUpcoming() {
        ...
        composeRule.awaitText("Rules")
        composeRule.onNodeWithText("Rules").assertIsDisplayed()
        composeRule.onNodeWithText("Settings").assertIsDisplayed()
        composeRule.onNodeWithText(WELCOME_HEADLINE).assertDoesNotExist()
     ...
     * permission -- skipping every optional step (no saved location, no notification
     * permission, no exact-alarm permission) must still complete onboarding
     * and hand off to Upcoming, exercising `OnboardingViewModel.finish`'s
     * locationJob-is-null path that [userCompletesOnboardingWithALocationAndReachesUpcoming]
     * never reaches.
     */
    @Test
...
/**
 * Advances past the notification-permission / exact-alarm steps without touching
 * real OS chrome (a system permission dialog, or the Settings activity the
 * exact-alarm intent opens) that an instrumented test running in-process can't
 * click deterministically: "Not now" where the step offers it, otherwise
 * "Continue" -- the label these steps fall back to once the permission is
 * already implied by the OS version (§10.2 notes both are conditional on API
 * level) or already granted.
 */
