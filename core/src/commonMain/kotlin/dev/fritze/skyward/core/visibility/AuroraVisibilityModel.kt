...
class AuroraVisibilityModel : VisibilityModel {
    ...
    override fun evaluate(occ: Occurrence, loc: SavedLocation, ctx: VisibilityContext): VisibilityResult {
        ...
        val (baseQuality, travelTarget) = when (payload.forecastKind) {
        ...
        return VisibilityResult(
            ...
            qualityAtNearestPoint = Quality.MARGINAL,
        )
    }

    /** THREE_DAY: Kp-threshold visibility boundary. Returns quality and, if not visible, a travel target. */
    private fun threeDayQuality(payload: AuroraPayload, loc: GeoPoint, absGmLat: Double): Pair<Quality, GeoPoint?> {
        ...
        if (quality != Quality.NONE) return quality to null
        ...
        return quality to target
    }

    /** NOWCAST: OVATION grid probability. Returns quality and, if not visible, the nearest probability>=50 cell. */
    private fun nowcastQuality(loc: GeoPoint, ctx: VisibilityContext): Pair<Quality, GeoPoint?> {
        val grid = ctx.ovationGrid ?: return Quality.NONE to null
        ...
        for (dLat in -latSpan..latSpan) {
            ...
            val cosLat = kotlin.math.cos(candidateLat * kotlin.math.PI / 180.0).coerceAtLeast(0.05)
