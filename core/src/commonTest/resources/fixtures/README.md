...
| `jpl_horizons_comet_elements.csv` | `ssd.jpl.nasa.gov/api/horizons.api` (`EPHEM_TYPE=ELEMENTS`), one osculating solution per comet, at perihelion | `tools/fixtures/fetch-horizons.py` |
| `jpl_horizons_comet_ephemerides.csv` | `ssd.jpl.nasa.gov/api/horizons.api` (`VECTORS` + `OBSERVER`), daily over perihelion ±182 days, for the same four comets | `tools/fixtures/fetch-horizons.py` |
