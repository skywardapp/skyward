...
// §15.4: the version the app reports about itself is derived from the same git
// tag as `versionCode`/`versionName`/`packageVersion`, not written out by hand.
//
// It used to be a hardcoded `const val APP_VERSION = "0.1.0"` in Main.kt with a
// comment asking whoever touched it to keep it in step with `packageVersion`.
// Nothing enforced that, and nothing could: auto-tag-main.yml cuts a tag on
// every push to main, so the real version moves without any commit touching it.
// By v0.1.41 the About section, and every §12 export file's `appVersion`
// header, still claimed 0.1.0. Generating the constant is the only arrangement
// that cannot drift — see ADR 0020.
val generateAppVersion by tasks.registering {
    description = "Generates AppVersion.kt with the git-tag-derived version (§15.4)."
    val version = rootProject.extra["skywardVersionName"] as String
    val outputDir = layout.buildDirectory.dir("generated/version/kotlin")
    inputs.property("version", version)
    outputs.dir(outputDir)
    doLast {
        val file = outputDir.get().asFile.resolve("dev/fritze/skyward/desktop/AppVersion.kt")
        file.parentFile.mkdirs()
        file.writeText(
            """
            // Generated by :desktopApp's generateAppVersion task — do not edit.
            package dev.fritze.skyward.desktop

            /**
             * §15.4's git-tag-derived version, as `git describe` renders it: the bare
             * tag ("0.1.41") on a release build, the tag plus "-<commits>-g<sha>" past
             * it. Shown in the About section, printed by `skyward --version`, written
             * into §12 export files, and — via that `--version` output — stamped into
             * the AppStream and AppImage metadata the packaging scripts produce.
             */
