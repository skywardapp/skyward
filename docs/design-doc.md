...
| Changes in 1.3 | §4.1's prohibition on sharing UI code between the two apps is removed at the owner's request: UI code **may** be shared, bounded now only by the three-module rule and by §15.3's Compose-free `:core`. The rule had rested on a citation that pointed at nothing — "(D1 note: frontends may differ)", a note that appears nowhere in D1, whose rationale is about Kotlin Multiplatform, nor anywhere else in this document — and its premise had already lapsed: ADR 0022 put §14's same four drawn views on both apps, so their draw bodies are identical by construction and the ban bought two copies of one drawing rather than two designs. No other constraint moves. §2 is untouched, there are still three Gradle modules (§4.1, §15.1), and `:core` `commonMain` still admits no Compose (§15.3). |
| Changes in 1.2 | Monetization dropped as a design consideration, not merely deferred: P6 rewritten, D12/D13 amended, and every rationale that rested on "keep a paid product possible" re-argued from D8 (GPL-3.0-or-later) and P3 (F-Droid) instead — §7.4.4, §12.3, §16, R5, R12. No technical constraint loosened; the companion monetization document is withdrawn. Also corrects a long-standing licensing error carried since 1.0: it is COBS' *NonCommercial* term that makes it GPL-incompatible, not ShareAlike — plain CC BY-SA 4.0 has been one-way GPLv3-compatible since 2015 (D12, §7.4.4, §16, R12). |
| Changes in 1.1 | Dual Android distribution (F-Droid + Play): D11–D13, §7.4 rewritten (COBS → JPL), §8.6, §10.2, §10.4, §11, §13, §15, §16, §17, §18, §19 updated. |
...
- **P6 — No commerce, and only free-licensed inputs.** Skyward has no commercial dimension, now or later: no billing code, no runtime licence or entitlement checks, no paid tiers, no ads, no donation links, no supporter unlock, no flavour split for revenue (D13). Separately, and *not* as a consequence of that: every dependency, data source and asset must carry a free licence with no field-of-use restriction — NonCommercial above all. That bar is owed to D8 and P3, not to any revenue plan: GPLv3 will not let us pass such a restriction on to recipients, and F-Droid does not ship non-free data. It is why comets come from JPL, not COBS (D12). §16 holds the table, and §17.5b the two enforcement paths — CI for Maven dependencies, review for everything else. Neither substitutes for the other.
...
- **No monetization, and no machinery for it** — no Play Billing dependency, no in-app purchases, no paid flavour, no licence/entitlement checks, no supporter unlock, no ads, no donation links in either build. Skyward is free everywhere and is not a staging ground for a paid successor: there is no companion monetization document and no paid product to keep design room for (D13).
...
| D12 | Comet data source | **JPL SBDB orbital elements + on-device Kepler propagation**; COBS removed entirely | COBS is CC BY-NC-SA 4.0, and the NonCommercial term is the disqualifier. GPLv3 guarantees every recipient the right to use and redistribute the work commercially, so an NC restriction is one we cannot carry into a GPLv3 release at all (D8) — and the ShareAlike relicensing route that rescues *plain* CC BY-SA 4.0, which Creative Commons designated GPLv3-compatible in 2015, has never been extended to the NC variants, so there is nothing to relicense through. Independently, NC data is not free, which bars it from an F-Droid build (P3). Neither ground has anything to do with revenue — D12 stands unchanged after D13's 1.2 amendment. JPL is a US Government work with no such limits, and elements-plus-propagation additionally yields sky positions, upgrading §8.6 from a magnitude-only check to a real visibility model. Cost: predicted, not observed, magnitudes (§7.4.4) |
| D13 | Monetization | **None, ever — and no machinery for it** | Owner's decision, amended 2026-08-23: v1.1 read "none in v1" and kept the door open for a later paid product; that option is dropped, and with it the companion monetization document. Skyward is free software with no commercial dimension. Product flavours exist only for the exact-alarm permission difference (§10.2, §15.1); they carry no billing, entitlement, or feature-gating code. Nothing else in the design changes — §16's licence bar, and the CI check that enforces its dependency half, are owed to D8 and P3, and are re-argued on those grounds in P6 |
...
Three Gradle modules only (`:core`, `:androidApp`, `:desktopApp`). Do **not** split `:core` into many modules; package-level separation inside `:core` is enough and keeps the build simple for the F-Droid server. **UI code may be shared between the two apps.** Versions 1.0–1.2 forbade that outright — *"UI code is intentionally **not** shared between the two apps (D1 note: frontends may differ)"* — and 1.3 withdraws the prohibition together with the note it cited, which never existed (see the change log above). Two rules outlive it and decide *where* shared UI may live: the module count in the sentence above, so sharing happens through a source directory both apps compile and never through a fourth module; and §15.3, which keeps Compose out of `:core`, so anything importing `androidx.compose.*` cannot go there. Small presentational helpers that are *pure functions* (formatting, colors for quality levels) still belong in `:core/format/`. Two facts bound what is practical rather than what is permitted, and both are worth establishing before a share is relied on. The apps compile against different Compose distributions — the AndroidX BOM on Android, Compose Multiplatform on the desktop — which agree on package names but are not the same artifacts, so a shared file has to be proven to compile under both and is un-shared again, rather than pinning either app, if they ever diverge on it. And sharing is permitted, not required: a phone layout and a desktop layout that merely resemble each other are two pieces of code, and folding them into one behind flags costs more than the duplication it removes. Share what is the same thing — the drawing of the same projection over the same data (ADR 0022) is the clear case.
...
**Why not COBS (D12).** The obvious source for comet brightness is COBS, which publishes *observed* visual magnitudes. It is licensed CC BY-NC-SA 4.0. The NonCommercial term disqualifies it twice over. (a) GPL-3.0 hands every recipient the right to use and redistribute commercially, so an NC restriction is one we cannot pass along (D8); COBS-derived data could therefore never be bundled or adapted, only ever fetched live and displayed untouched. Note the precise reason ShareAlike offers no way out: CC designated GPLv3 as a compatible licence for plain CC BY-SA 4.0 in 2015, but no non-CC licence has ever been designated compatible with the *NC* variants, so the relicensing path simply does not exist here. (b) NC data is not free, which keeps it out of an F-Droid-distributed app (P3) regardless of the GPL question. Rather than build a feature on a source the app's own licence cannot carry, v1 uses JPL and accepts predicted magnitudes. §7.4.4 states the accuracy cost honestly.
...
- **Location prominent disclosure (required by Play, harmless on F-Droid):** before the first `ACCESS_COARSE_LOCATION` runtime prompt, show a full-screen disclosure stating what is accessed (approximate location), why (to compute visibility from where you are), and that it stays on-device **except** for an approximate region — derived from all saved locations, never exact coordinates — optionally included in NASA EONET queries (§7.7) to narrow that source's response (ADR 0016). The permission is entirely optional — manual location entry is the default path and the app must be fully usable without ever granting it.
...
- **Forward-compatibility note:** keep the format application-agnostic — no field may encode which app or store produced it beyond the informational `appVersion`. It costs nothing now and it is what lets a user's data move between the F-Droid and Play builds should their signatures ever diverge (§15.4), and lets a downstream fork or successor read the file at all, which is the least a GPL app owes its users. (v1.1 justified this rule by a possible migration to a paid app; D13 withdrew that, and the rule stands without it.)
...
**Every row below must be free-licensed and GPL-3-compatible (P6).** A dependency or dataset carrying a field-of-use restriction — NonCommercial above all, but equally SSPL, BUSL or a Commons Clause rider — does not enter this table, regardless of how convenient it is; that rule is what removed COBS (D12). The bar is owed to D8 and P3, not to any commercial plan: GPLv3's no-further-restrictions clause forbids passing such terms on to recipients, and F-Droid will not distribute non-free data. Before adding any new data source, check the licence against this bar first and the technical fit second.
...
| ~~COBS comet data~~ | ~~CC BY-NC-SA 4.0~~ | **EXCLUDED (D12)** | The NonCommercial term cannot be carried into GPLv3, which grants recipients commercial use (D8), and CC has designated no compatible licence for the NC variants the way it did for plain BY-SA 4.0 — so there is no relicensing route. NC data is also not free, which bars an F-Droid release (P3/P6). Do not reintroduce, not even as an "offline fallback" or a quick fix for §7.4.4's accuracy limits. If observed magnitudes become necessary, obtain a written free-licence grant first — permission for *us* to use the data commercially is not enough, because what GPLv3 obliges us to convey is that freedom to every recipient — or find a permissively licensed source (R12) |
...
- A dependency-licence report fails the build on any dependency whose licence is not on the allowlist, and on any whose licence it cannot read at all. Both cases are rescued by the same thing and nothing else: a recorded manual verdict for that `group:module` in `licenseUnknownExceptions` — the escape hatch for a stub POM, and equally for a readable licence like EPL-2.0 whose GPL-compatibility election lives outside the POM. A denylisted licence is never exceptable. This is the automated enforcement of P6/§16.
...
| R5 | **GPL choice locks out future non-GPL reuse of core** | Strategic; largely moot since D13's 1.2 amendment | Accepted trade-off (D8), and with monetization dropped outright there is no remaining scenario in this project that wants non-GPL reuse. Kept on the register only so the fact stays known to a downstream fork: the single GPL-infecting asset is Stellarium's shower catalog, replaceable by a self-curated IAU-MDC-derived dataset |
...
| R12 | **No permissively licensed source of *observed* comet magnitudes** | Comet feature stays prediction-only | v1.1: ask Observatory Črni Vrh (COBS) to re-licence the data we would use under free, GPL-3-compatible terms — note this is a *higher* bar than the "permission covering commercial use" v1.1 asked for, because a grant to *us* leaves the data non-free for our recipients, and conveying their freedom is exactly what GPLv3 requires; charging nothing does not cure it — or evaluate MPC (licence status murky, verify before relying on it). Until one lands, do not reintroduce COBS (D12) |
