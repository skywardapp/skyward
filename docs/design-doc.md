...
- **P6 — No commerce, and only free-licensed inputs.** Skyward has no commercial dimension, now or later: no billing code, no licence checks, no paid tiers, no ads, no donation links, no supporter unlock, and no flavour split for revenue (D13). Separately, and *not* as a consequence of that: every dependency, data source and asset must carry a free licence with no field-of-use restriction — NonCommercial above all. That bar is owed to D8 and P3, not to any revenue plan. GPLv3 does not let us hand recipients a work carrying restrictions we are not free to pass on, and F-Droid does not ship non-free data. It is why the comet source is JPL, not COBS (D12), and it is enforced in CI by `checkDependencyLicenses` (§16, §17.5b).
...
| ~~COBS comet data~~ | ~~CC BY-NC-SA 4.0~~ | **EXCLUDED (D12)** | ShareAlike is GPL-incompatible (D8) and NonCommercial is not a free licence, so the data cannot ride along in an F-Droid or GPLv3 release (P3/P6). Do not reintroduce, not even as an "offline fallback" or a quick fix for §7.4.4's accuracy limits. If observed magnitudes become necessary, obtain a written free-licence grant first — bare permission to use the data commercially is *not* enough, since it is the ShareAlike and non-free terms that bind — or find a permissively licensed source (R12) |
