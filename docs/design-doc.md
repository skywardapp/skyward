...
- **P6 — No commerce, and only free-licensed inputs.** Skyward has no commercial dimension, now or later: no billing code, no runtime licence or entitlement checks, no paid tiers, no ads, no donation links, no supporter unlock, and no flavour split for revenue (D13). Separately, and *not* as a consequence of that: every dependency, data source and asset must carry a free licence with no field-of-use restriction — NonCommercial above all. That bar is owed to D8 and P3, not to any revenue plan. GPLv3 does not let us hand recipients a work carrying restrictions we are not free to pass on, and F-Droid does not ship non-free data. It is why the comet source is JPL, not COBS (D12). Note what enforces which half: `checkDependencyLicenses` (§16, §17.5b) reads the POM-declared licence of every *Maven dependency* on a shipped classpath and fails the build on any licence it cannot place — whether the POM declares none at all, or declares one that simply is not on the allowlist — unless a recorded manual verdict for that `group:module` sits in `licenseUnknownExceptions`; a *denylisted* licence is never exceptable. It sees nothing else. Bundled assets (Stellarium's `showers.json`, the Natural Earth binary, the vendored `astronomy.kt`) and runtime data sources (SWPC, JPL, EONET) are outside its reach and are cleared by review against §16's table before they are added. Both halves are load-bearing; neither substitutes for the other.
